...
function getCookie(name){
    ...
    session.defaultSession.cookies.get({url : ip},function(error,cookies){
        for(var i = 0;i < cookies.length;i++){
            var row = cookies[i];
            if(row.name == name) value = row.value;
        }
    });
    return value;
}